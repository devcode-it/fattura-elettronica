...
class IscrizioneREA extends Elemento {
    ...
    public function __construct(?string $Ufficio = null, ?string $NumeroREA = null, ?float $CapitaleSociale = null, ?string $SocioUnico = null, ?string $StatoLiquidazione = null) {
        ...
        if (!is_null($Ufficio)) $this->setUfficio($Ufficio);
		if (!is_null($NumeroREA)) $this->setNumeroREA($NumeroREA);
		if (!is_null($CapitaleSociale)) $this->setCapitaleSociale($CapitaleSociale);
		if (!is_null($SocioUnico)) $this->setSocioUnico($SocioUnico);
		if (!is_null($StatoLiquidazione)) $this->setStatoLiquidazione($StatoLiquidazione);
    }
