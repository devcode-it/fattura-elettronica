...
use DevCode\FatturaElettronica\Semplificata\FatturaElettronicaBody\DatiGenerali\DatiGeneraliDocumento\DatiBollo;
use DevCode\FatturaElettronica\Semplificata\FatturaElettronicaBody\DatiGenerali\DatiGeneraliDocumento\DatiCassaPrevidenziale;
use DevCode\FatturaElettronica\Semplificata\FatturaElettronicaBody\DatiGenerali\DatiGeneraliDocumento\DatiRitenuta;
use DevCode\FatturaElettronica\Semplificata\FatturaElettronicaBody\DatiGenerali\DatiGeneraliDocumento\ScontoMaggiorazione;
...
class DatiGeneraliDocumento extends Elemento {
    ...
    public function __construct(?string $TipoDocumento = null, ?string $Divisa = null, null|string|Carbon|\DateTime $Data = null, ?string $Numero = null, ?float $ImportoTotaleDocumento = null, ?float $Arrotondamento = null, ?string $Causale = null, ?string $Art73 = null) {
        ...
        if (!is_null($TipoDocumento)) $this->setTipoDocumento($TipoDocumento);
		if (!is_null($Divisa)) $this->setDivisa($Divisa);
		if (!is_null($Data)) $this->setData($Data);
		if (!is_null($Numero)) $this->setNumero($Numero);
		if (!is_null($ImportoTotaleDocumento)) $this->setImportoTotaleDocumento($ImportoTotaleDocumento);
		if (!is_null($Arrotondamento)) $this->setArrotondamento($Arrotondamento);
		if (!is_null($Causale)) $this->setCausale($Causale);
		if (!is_null($Art73)) $this->setArt73($Art73);
    }
