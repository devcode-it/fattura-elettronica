...
use DevCode\FatturaElettronica\Semplificata\FatturaElettronicaHeader\CedentePrestatore\DatiAnagrafici\Anagrafica;
use DevCode\FatturaElettronica\Semplificata\FatturaElettronicaHeader\CedentePrestatore\DatiAnagrafici\IdFiscaleIVA;
...
class DatiAnagrafici extends Elemento {
    ...
    public function __construct(?string $CodiceFiscale = null, ?string $AlboProfessionale = null, ?string $ProvinciaAlbo = null, ?string $NumeroIscrizioneAlbo = null, null|string|Carbon|\DateTime $DataIscrizioneAlbo = null, ?string $RegimeFiscale = null) {
		...
		$this->DataIscrizioneAlbo = new Data(true);
		$this->RegimeFiscale = new Testo(false, 4, 4, 1);
        if (!is_null($CodiceFiscale)) $this->setCodiceFiscale($CodiceFiscale);
		if (!is_null($AlboProfessionale)) $this->setAlboProfessionale($AlboProfessionale);
		if (!is_null($ProvinciaAlbo)) $this->setProvinciaAlbo($ProvinciaAlbo);
		if (!is_null($NumeroIscrizioneAlbo)) $this->setNumeroIscrizioneAlbo($NumeroIscrizioneAlbo);
		if (!is_null($DataIscrizioneAlbo)) $this->setDataIscrizioneAlbo($DataIscrizioneAlbo);
    ...
    public function setDataIscrizioneAlbo(null|string|Carbon|\DateTime $value) {
