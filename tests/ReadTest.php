...
function xml_is_equal(SimpleXMLElement $xml1, SimpleXMLElement $xml2, $text_strict = false) {
    ...
    if ($text_strict) {
        if ("$xml1" != "$xml2") return "mismatched text content (strict, '{$xml1}' vs '{$xml2}')";
    } else {
        if (trim("$xml1") != trim("$xml2")) return "mismatched text content ('{$xml1}' vs '{$xml2}')";
    }
    ...
    if ($diff != "[]") return "mismatched children count ($diff)";		// xml2 has less or more children names (we don't have to search through xml2's children too)
